...
\large Li \textit{et al.} (2017) / Farquarson \& Li comparison}
...
forward model (\texttt{comparision.ipynb}, validating against Farquarson \& Li, 2017). This note gives
the fix first, then the analysis that justifies it.

\begin{enumerate}\setlength\itemsep{0.1em}
  \item \textbf{The fix is one line} in \texttt{forward\_farquarson.py}: a gentler time-step ramp
  ($\times2\to\times1.4$). It moves the 3D octree response from \textbf{$+7$--$24\,\%$ high} to within
  \textbf{$\sim\pm2\,\%$} of analytic on the \emph{same} mesh (Section~\ref{sec:fix}).
  \item \textbf{Root cause: the time-stepping, not the mesh.} The original $\times2$ ramp (55 steps) is too
  coarse for backward-Euler. Refining the time-steps alone removes the bias; refining the mesh alone barely
  moves it (Section~\ref{sec:why}).
  \item \textbf{The SimPEG 1D layered solution was never the problem.} \texttt{Simulation1DLayered}
  reproduces the closed-form Ward \& Hohmann response to $\sim$5 significant figures (circular loop); it is
  the trustworthy reference (Section~\ref{sec:1d}).
  \item \textbf{The residual $\sim$1.8\,\% is real square-vs-circle geometry}, not modelling error
  (Section~\ref{sec:geom}); driving \emph{every} channel below 1\,\% is a genuine convergence study, not a
  one-line change (Section~\ref{sec:onepct}).
\end{enumerate}
...
\section{The fix (applied to \texttt{forward\_farquarson.py})}
\label{sec:fix}
...
\texttt{Simulation3DElectricField}. Its original time discretization is a coarse geometric ramp --- 55
steps whose size doubles each level (\SI{1}{\micro s}\,$\to$\,\SI{1.02}{ms}). Doubling is too aggressive
for the backward-Euler scheme to resolve the diffusing response, biasing $\dbdt$ high. The single line
setting \texttt{time\_steps} was replaced with a gentler ramp:
\begin{center}\small
...
$\sim$2\,\% of analytic on the \emph{same} \texttt{[10,10,5]} mesh (Figure~\ref{fig:farq});
\texttt{increase\_rate=1.5} (90 steps) is a cheaper $\sim$5\,\% compromise, and the mesh does not need to
change. The committed \texttt{half\_tem\_faquarson.csv} was regenerated with this corrected time-stepping
(mean ratio 0.996, range 0.955--1.019 vs analytic); the original is retained as
\texttt{half\_tem\_faquarson.csv.orig\_backup}.
...
\section{Why it works: time-stepping, not mesh}
\label{sec:why}
...
\textbf{finest time (the fix)}  & coarse \SI{28.5}{k} & $\times1.4$, 156 & \textbf{0.996} & 0.96--1.02 \\
...
\section{The 1D layered solution is the correct reference}
\label{sec:1d}
...
--- the expected geometric convergence of a square loop to its equal-area circle (Section~\ref{sec:geom}).
The 1D code is therefore \emph{not} the source of the discrepancy and is the trustworthy reference
(Figure~\ref{fig:diag}).
...
\begin{figure}[h]\centering
  ...
  $\sim$2\,\%; the original 3D octree run (blue) is 7--24\,\% high and jagged.}
  \label{fig:diag}
\end{figure}
...
\texttt{halfspace\_validation\_and\_timestep\_fix.ipynb} (self-contained; $\sim$6--7\,min runtime for the
seven 3D octree solves; it auto-selects whatever direct solver is installed). The convergence harness is
\texttt{convergence.py} and the CSV regeneration is \texttt{regen\_half.py}.
